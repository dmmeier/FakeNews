...
def gen_info(s, X_array, which_X, t_array, tau, C, m):
    info_processes = []
    for kx in range(len(tau)): #number of candidates
        
        #flip C for candidate two, so that fake news doesn't undo itself
        ...
        for ix in range(len(tau_current)): #number of processes
            ...
            plot_info(info_path, t_array, C_proc, m, which_tau_proc)
            cand_info.append(info_path)
        info_processes.append(cand_info)
    return info_processes #list of lists [ [candA infoproc1, candA infoproc2, ...], [candB infoproc1, ...], ...]
...
def Par_allpred_helper(i, t):
    (t_array, s, C, m, mu, prior, X_array, T, N, which_X, tau, info_processes, noises, MC_measure) = t
    index = i
    print("Dealing with index{}".format(i))
    A = (t_array, s, C, m, mu, prior, X_array, T, N, which_X, tau, info_processes, index, noises, MC_measure)
    res = all_predictions(A)
    return res

def get_all_pred(B):
    (t_array, s, C, m, mu, prior, X_array, T, N, which_X, tau, info_processes, noises, MC_measure) = B
    results = Par_prediction_all_curve(B)
    
    output = open('result.pkl', 'wb')
    pickle.dump((results, B), output) 
    output.close()
...
def plotting_pred_all():
    ...
    for kx in range(len(tau)): #number of candidates
            ...
            pred_can = pred_curr[kx]
            for ix in range(len(tau_current)): #number of processes
                plt.plot(cand["cand{}proc{}m2".format(kx, ix)], label = "Model2")
                plt.plot(cand["cand{}proc{}ig".format(kx, ix)], label = "Ignorant")
                plt.plot(cand["cand{}proc{}true".format(kx, ix)], label = "True")
                plt.legend()
                plt.title("Candidate {}, process {}".format(kx, ix))
                plt.show()
    ...
    if len(tau) == 2: #number of candidates is 2
        ...
        W_array = np.random.randn(Nproc, int(Npop/2)) + (-1) * np.ones((Nproc, 1)) + 0.5*np.ones((Nproc, 1))
        W_array_other = np.random.randn(Nproc, int(Npop/2)) + np.ones((Nproc, 1))
        W_array = np.concatenate((W_array, W_array_other), axis = 1)
        votes_m2 = []
        votes_ig = []
        votes_true =[]
        ...
        if len(tau) ==2 and len(tau_current) == 3: #have three difference processes; can visualize
            ax.plot(diff_m2_array[0, :], diff_m2_array[1, :], diff_m2_array[2, :], label = 'Model2')
            ax.plot(diff_ig_array[0, :], diff_ig_array[1, :], diff_ig_array[2, :], label = 'Ignorant')
            ax.plot(diff_true_array[0, :], diff_true_array[1, :], diff_true_array[2, :], label = 'True')
            plt.legend()
            plt.show()
        ...
        votes_m2 = np.sum(votes_m2_array, axis = 0, keepdims = True)/Npop
        votes_ig = np.sum(votes_ig_array, axis = 0 , keepdims = True)/Npop
        votes_true = np.sum(votes_true_array, axis = 0 , keepdims = True)/Npop
                
        plt.plot(votes_m2[0, int(Ngrid/5):], label = "Model2")
        plt.plot(votes_ig[0, int(Ngrid/5):], label = "Ignorant")
        plt.plot(votes_true[0, int(Ngrid/5):], label = "True")
        #remove the very noisy beginning of the plot
        plt.legend()
        plt.title("Voting proportions")
        plt.show()
        
    if len(tau_current) == 2 and len(tau) == 2: #number of processes is 2 -->> can visualize
        plt.plot(differences["proc0m2"], differences["proc1m2"], label = "Model2")
        plt.plot(differences["proc0ig"], differences["proc1ig"], label = "Ignorant")
        plt.plot(differences["proc0true"], differences["proc1true"], label = "True")
        plt.legend()
        plt.title("X_A - X_B")
        plt.show()
...
Npop = 1000 #size of population modelled


par_root = 16 #number of root finding points we compute at each step
high_precision_root_finding = 0 #if 1, then do par_root search steps parallelized; otherwise approximate derivative using only 3 searches
frac = 0.3 #Used in high precision root finding (how much of prev_prior - new prior (using approximate method) interval is searched over)
...
s = [[.1, .1, .1], [.1, .1, .1]] #information flow rates [(candA factor1, candA factor2), (candB factor1, candB factor2), etc]
C = [[3, 3, 3], [-3, -3, -3]] #magnitude of fake news
...
X_array = [[np.array([[-1, 1]]), np.array([[-1, 1]]), np.array([[-1, 1]])] , [np.array([[-1, 1]]), np.array([[-1, 1]]), np.array([[-1, 1]])]] #the true underlying random variable we attempt to detect
T = 1 #final time
...
tau = [[np.array([[0.05, 0.2, 0.4, 0.1]]), np.array([[0.2, 0.2, 0.3, 0.1]]), np.array([[0.2, 0.2, 0.3, 0.1]])], [np.array([[0.1, 0.3, 0.4, 0.1]]), np.array([[0.1, 0.1, 0.3, 0.3]]), np.array([[0.1, 0.1, 0.3, 0.3]])]] #array of waiting times (interarrival times) if these are fixed (in case of taugen being 'deterministic' or 'predcurve', ot 'random')
Ngrid = 512 #number of steps discretizing time for the prediction curves (in case taugen is 'predcurve' or 'macro' or 'fifty'); that is, resolution of predcurves (Ngrid predictions are required)
Nproc = len(which_X[0]) #number of processes per candidate
...
for kx in range(len(tau)): #number of candidates
    ...
    for ix in range(len(temp)): #number processes
        templist.append(temp[ix].shape[1])
        templist_MC.append(integration_upper_bound**temp[ix].shape[1])
    noises.append(templist) #number of fake news items
    MC_measure.append(templist_MC) #MC measures for each process of each candidate

t_array = gen_t_array(T, N) #generate t_array
    
        
#generate information processes
info_processes = gen_info(s, X_array, which_X, t_array, tau, C, m)  
    
time_of_conditioning = 0.9
index = int(N*time_of_conditioning)

#A = (t_array, s, C, m, mu, prior, X_array, T, N, which_X, tau, info_processes, index, noises, MC_measure)
#predictions = all_predictions(A) 

B = (t_array, s, C, m, mu, prior, X_array, T, N, which_X, tau, info_processes, noises, MC_measure)
#curve = Par_prediction_all_curve(B)
